...
Revision    : $Revision: 1.6 $, $Date: 2002/04/18 21:17:54 $
...
$Revision: 1.6 $, $Date: 2002/04/18 21:17:54 $, $Author: mlibersan $
...
function doOk() {
  setForm();
  unlockFields();
  soumettre('<%= request.getContextPath() + "/societe/narration/add.do"%>');
}

function doCancel() {
  if (fenetreNarration.innerHTML == document.forms(0).narrationAvecFormat.value ){
    window.location = '<%=request.getContextPath()%>/societe/show.do?site=<bean:write name="narration" property="societe.site"/>&cle=<bean:write name="narration" property="societe.cle"/>';
  }else{
...
<body onKeyDown="return toucheCTRLNarration();"
  ...
  <html:form action="/societe/narration/add"  >
    <html:hidden  name="narration" property="societe.cle" />
    <html:hidden  name="narration" property="societe.site" />
    ...
    <tiles:insert page="/templates/narrations/tpl_narrations_formulaire.jsp" flush="true">
		<tiles:put name="urlSecuriteSauvegarde" value="/societe/narration/add.do" />
