...
Revision    : $Revision: 1.9 $, $Date: 2002/04/19 20:00:14 $
...
$Revision: 1.9 $, $Date: 2002/04/19 20:00:14 $, $Author: mlibersan $
...
function doOk() {
  unlockFields();
  setForm();
  soumettre('<%= request.getContextPath() + "/societe/narration/update.do"%>');
}

function doCancel() {
//--  if (document.forms(0).HTMLEditor.isChanged() == false){
  if (fenetreNarration.isContentEditable == false){
    window.location = '<%=request.getContextPath()%>/societe/show.do?site=<bean:write name="narration" property="lienSite"/>&cle=<bean:write name="narration" property="lien"/>';
  }else{
...
function doApprobation() {
  setForm();
  soumettre('<%= request.getContextPath() + "/societe/narration/approve.do"%>');
}

function doModification() {
  setForm();
  soumettre('<%= request.getContextPath() + "/societe/narration/modified.do"%>');
}
...
function doPrint() {
  windowOpenLocation('<%= request.getContextPath() %>/narrations/w_impression_narration_frameset_societe.jsp');
}
...
<body
  ...
  <html:form action="/societe/narration/update"  >
    ...
    <tiles:insert page="/templates/narrations/tpl_narrations_formulaire.jsp" flush="true">
		<tiles:put name="urlSecuriteSauvegarde" value="/societe/narration/update.do" />
		<tiles:put name="urlSecuriteApprobation" value="/societe/narration/approve.do" />
		<tiles:put name="urlSecuriteModification" value="/societe/narration/modified.do" />
