...
function show_date_time(str_target, str_datetime) {
	var arr_months = ["January", "February", "March", "April", "May", "June",
    "July", "August", "September", "October", "November", "December"];
  var week_days = ["Su", "Mo", "Tu", "We", "Th", "Fr", "Sa"];
	var n_weekstart = 0; // day week starts from (normally 0 or 1)

	var dt_datetime = (str_datetime == null || str_datetime =="" ?  new Date() : str2dt_dtp(str_datetime));
  ...
  dt_next_year.setYear(dt_datetime.getFullYear()+1);
	...
	var str_buffer = new String (
		...
		"  <table cellspacing=\"1\" cellpadding=\"3\" border=\"0\" width=\"100%\" bgcolor=\"#4682B4\" >" +
		"    <tr>	" +
		"      <td><a href=\"javascript:show_date_time('" +
           str_target + "', '" + dt2dtstr_dtp(dt_prev_year) + "' + document.forms(0).timeCalendar.value);" +
           "  \">" +
           "<b style='text-decoration: none; color: #87CEFA;'>&lt;&lt;</b></a></td>" +
    "      <td><a href=\"javascript:show_date_time('" +
      		 str_target + "', '" + dt2dtstr_dtp(dt_prev_month) + "' + document.forms(0).timeCalendar.value);\">" +
		...
		"      	<td align=\"right\"><a href=\"javascript:show_date_time('"
        		+ str_target + "', '" + dt2dtstr_dtp(dt_next_month) + "' + document.forms(0).timeCalendar.value);\">" +
        		"<b style='text-decoration: none; color: #87CEFA;'> " +
        		"&gt;</b></a></td>" +
		"       <td align=\"right\"><a href=\"javascript:show_date_time('"
            + str_target + "', '" + dt2dtstr_dtp(dt_next_year) + "' + document.forms(0).timeCalendar.value);\">"+
            "<b style='text-decoration: none; color: #87CEFA;'>" +
            "&gt;&gt;</b></a></td>"+
    "    </tr>" +
    "  </table> " +
...
var dt_current_day = new Date(dt_firstday);
	...
	while (dt_current_day.getMonth() == dt_datetime.getMonth() ||
		...
		for (var n_current_wday=0; n_current_wday<7; n_current_wday++) {
				...
				if (dt_current_day.getMonth() == dt_datetime.getMonth())
					...
					".value='" + dt2dtstr_dtp(dt_current_day) + "' + document.forms(0).timeCalendar.value; document.all['CALENDAR_DIV'].style.visibility='hidden';" + str_target + ".blur();\" >" +
	...
	str_buffer +=
		...
		"<font color=\"White\" face=\"tahoma, verdana\" size=\"2\">" +
		"Time: <input type=\"text\" name=\"timeCalendar\" value=\"" + dt2tmstr_dtp(dt_datetime) +
...
function str2dt_dtp (str_datetime) {
	//Masque de saisie.
	...
	//return (new Date ( 2001, 12-1, 25, 00, 00, 00 ));
	var aDate = new Date(str_datetime);
	var aYear = aDate.getFullYear();
	var aMonth = aDate.getMonth();
	var aDay = aDate.getDate();
	var anHour = aDate.getHours();
	var aMinute = aDate.getMinutes();
	var aSecond = aDate.getSeconds();
	return (new Date (aYear, aMonth, aDay, anHour, aMinute, aSecond));
}

/* */
function dt2dtstr_dtp (dt_datetime) {
  var aMonth = "";
...
function dt2tmstr_dtp (dt_datetime) {
