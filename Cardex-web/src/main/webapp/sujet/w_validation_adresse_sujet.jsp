...
function doOk() {
	unlockFields();
    soumettre('<%= request.getContextPath() + "/sujet/adresse/choixValidation.do"%>');
}

function doRefresh() {
	unlockFields();
    soumettre('<%= request.getContextPath() + "/sujet/adresse/refreshValidation.do"%>');
}

function doCancel() {
	unlockFields();
    soumettre('<%=request.getContextPath()+"/sujet/adresse/retourValidation.do"%>');
}
...
<table align="center" height="550" border="0" cellpadding="5" cellspacing="0" bgcolor="#ffffff">
    <tr>
       <td align="center">
          ...
          <html:form action="/sujet/adresse/choixValidation" >
            <tiles:insert page="/templates/adresses/tpl_validation_formulaire.jsp" flush="true">
				<tiles:put name="soumettreValidation" value="/sujet/adresse/validation.do" />
				<tiles:put name="soumettreChoix" value="/sujet/adresse/choixValidation.do" />
			</tiles:insert>	
          </html:form>
       </td>
   </tr>
