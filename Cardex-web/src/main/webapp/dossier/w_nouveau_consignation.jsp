...
function doRefresh() {
  unlockFields();
  soumettre('<%= request.getContextPath() + "/dossier/consignation/create/refresh.do"%>');
}
...
function doAjouter() {
  unlockFields();
  soumettre('<%= request.getContextPath() + "/dossier/consignation/ajouter.do"%>');
}
...
<input type="hidden" name="clipboard" value="required">
  <html:form action="/dossier/consignation/add"  >
    ...
    <tiles:insert page="/templates/consignation/tpl_consignation_formulaire.jsp" flush="true">
		<tiles:put name="urlSecuriteSauvegarde" value="/dossier/consignation/add.do" />
