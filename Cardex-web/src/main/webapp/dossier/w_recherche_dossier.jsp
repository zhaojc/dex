...
//Impression d'un rapport personnalisé
	...
	if(document.forms(0).siteOrigine.value == ""){
	   alert("<bean:message key='cardex_required_site'/>");
		...
		if( rapport == '128'){ //RapportsConfiguration.RAPPORT_STATISTIQUE_DOSSIER_CATEGORIE
			if(document.forms(0).type.value == ""){
	   			alert("<bean:message key='cardex_required_type'/>");
			}else{
				ouvrirRapport();
			}
		}else{
		    if( rapport == '129' || rapport == '478'){//RapportsConfiguration.RAPPORT_STATISTIQUE_DOSSIER_ANNUEL, RapportsConfiguration.RAPPORT_STATISTIQUE_DOSSIER_TYPE
				if(document.forms(0).nature.value == ""){
		   			alert("<bean:message key='cardex_required_nature'/>");
				}else{
					ouvrirRapport();
				}
		    }else{
		    	if(rapport == '479' || rapport == '480' || rapport == '482' || rapport == '483'){ //RapportsConfiguration.RAPPORT_CUMULATIF_ESPACEJEUX et RapportsConfiguration.RAPPORT_HEBDOMADAIRE_ESPACEJEUX et RapportsConfiguration.RAPPORT_CUMULATIF et RapportsConfiguration.RAPPORT_HEBDOMADAIRE 
					if(document.forms(0).genre.value == ""){
	   					alert("<bean:message key='cardex_required_genre'/>");
					}else{
					   var genre = document.forms(0).genre.value;
					   var siteOrigine = document.forms(0).siteOrigine.value;
					   var dateDebut = document.forms(0).dateDebutDu.value;
					   var dateFin = document.forms(0).dateDebutAu.value;	
					   var tableValeurRapport = "<%=GlobalConstants.TableValeur.RAPPORT_RECHERCHE_DOSSIER%>";
					   var url = "<%=request.getContextPath()%>/RapportAffichagePDF?choixRapport=" + rapport + "&siteOrigine=" + siteOrigine + "&dateDebutDu=" + dateDebut + "&dateDebutAu=" + dateFin + "&tableValeurRapport=" + tableValeurRapport + "&genre=" + genre;
					   window.open(url, 'rapport', 'left=0,top=0,width=' + document.body.clientWidth + ',height=' + document.body.clientHeight + ',menubar=no,toolbar=no,resizable=yes');
					}
		    	}else{
		    		if( rapport == '481'){ //GlobalConstants.ChoixRapport.TEMPS_CONSACRE
						if(document.forms(0).genre.value == ""){
				   			alert("<bean:message key='cardex_required_genre'/>");
						}else{
						   var siteOrigine = document.forms(0).siteOrigine.value;
						   ...
						   var url = "<%=request.getContextPath()%>/AffichagePDF?RAPPORT=" + rapport + "&SITE=" + siteOrigine + "&GENRE=" + genre + "&NATURE=" + nature + "&UTILISATEUR=" + userCardex + "&DATE_DEBUT=" + dateDebut + "&DATE_FIN=" + dateFin + "&CATEGORIE=" + categorie;
						   window.open(url, 'rapport', 'left=0,top=0,width=' + document.body.clientWidth + ',height=' + document.body.clientHeight + ',menubar=no,toolbar=no,resizable=yes');
						}
					}else{
						   var siteOrigine = document.forms(0).siteOrigine.value;
	       ...
	       //Pour empêcher le rapport de s'exécuter à rafraîchissement de la page, on met une valeur nulle dans la liste des rapports.
		   document.forms(0).choixRapport.value = "";
	}
}
