<%-- --------------------------------------------------------------------------
Use case    : Création d'une évaluation par le Comité de vigilance.
Description : Évaluation d'un individu qui fait l'objet d'une surveillance pour
			  différentes raisons.
Author(s)   : $Author: mlibersan $, abruno-boucher
Revision    : $Revision: 1.4 $, $Date: 2012/06/04 20:00:18 $
...
$Revision: 1.4 $, $Date: 2002/04/19 20:00:18 $, $Author: mlibersan $
Création.
--------------------------------------------------------------------------- --%>
...
<SCRIPT language="JavaScript" type="text/javascript">
	...
	function doOk() {
	    ...
	    unlockFields();
	    soumettre('<%=request.getContextPath()%>/dossier/evaluation/add.do');
	}
	...
	function doAjoutFrequence(indexMise) {
		...
		soumettre('<%=request.getContextPath()%>/evaluation/ajouterFrequence.do');
	}
	...
	function doRetraitFrequence(indexMise, indexFrequenceVisites) {
		...
		soumettre('<%=request.getContextPath()%>/evaluation/retirerFrequence.do');
	}
	...
	function doAjoutMiseEvaluation() {
	    ...
	    selectMultiboxesetatsChoisis();
	    selectMultiboxesproposChoisis();
		soumettre('<%=request.getContextPath()%>/evaluation/ajouterMiseEvaluation.do');
	}
	...
	function doRetraitMiseEvaluation(indexMise) {
	    ...
	    selectMultiboxesproposChoisis();
	    document.forms(0).indexMise.value = indexMise;
		soumettre('<%=request.getContextPath()%>/evaluation/retirerMiseEvaluation.do');
	}
</SCRIPT>
          ...
          <html:form action="/dossier/evaluation/add" >
		    ...
		    <jsp:include page="/templates/tpl_erreur.jsp" flush="true" />       
          	<tiles:insert page="/templates/evaluation/tpl_evaluation_formulaire.jsp" flush="true">
          		<tiles:put name="soumettreURLSauvegarde" value="/dossier/evaluation/add.do" />
          	</tiles:insert>
