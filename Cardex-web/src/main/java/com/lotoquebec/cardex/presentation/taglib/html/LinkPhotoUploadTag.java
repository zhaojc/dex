 ...
 * @version $Revision: 1.7 $ $Date: 2002/04/22 18:25:01 $
 */
public class LinkPhotoUploadTag extends LinkCardexTag {
	...
	return (SKIP_BODY);
      ...
      if (source != null ) {
        ...
        if (bean == null) {
            JspException e = new JspException("Unable to find bean name '"+name+"' in LinkPhotoUploadTag.");
        ...
        if (bean instanceof  DossierHtmlForm){
            ...
            //La confidentialit� est ajout�e pour servir de valeur dans le champ
            //confidentialit� de la table LMM_LIEN_MULTIMEDIA. Ce champ est n�cessaire
            //pour des raisons de filtrage des donn�es dans la r�plication.
            ...
            pageContext.setAttribute("source.parameters",parameters);
              ...
              JspException e = new JspException("Invalid type '"+bean.getClass().getName()+"' for bean name '"+name+"' in LinkPhotoUploadTag, the bean type must be '"+DossierHtmlForm.class.getName()+"' or '"+SujetHtmlForm.class.getName()+"'.");
              tagUtils.saveException(pageContext, e);
              throw e;
        }
      }
