...
/**
 * Seulement utilisez cette classe pour les rapports de présentation.
 * Bref, ceux qui sont appelé par la présentation sans passer par les composants
 * @author levassc
 *
 */
public abstract class GenererRapportForm {
	 ...
	 */
	protected abstract JRDataSource construireDataSource(CardexAuthenticationSubject subject, ValidatorForm validatorForm, Connection connection) throws BusinessResourceException, BusinessException;
	 ...
	 * @return
	 */
	public JasperPrint executer(CardexAuthenticationSubject subject, ValidatorForm validatorForm, Locale locale) throws BusinessException, JRException{
		...
		try {
			...
			JRDataSource dataSource = construireDataSource(subject, validatorForm, connection);
			Map<String,Object> parameters = construireParametres(subject, validatorForm, connection);
