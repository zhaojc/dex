...
public class RapportNumeroSerieDenomination_CDX_0153{
	...
	public static PreparerSQL construireSQL(ConsignationRapportVO consignationRapportVO) {
		...
		query.append("SELECT s.v_si_site as SITE, c.v_cn_numero_serie as NUMERO_SERIE, ");
		...
		query.append("FROM CN_CONSIGNATION C, tr_traduction tdn, si_site s ");
		query.append("where C.L_SI_CLE = ? ");
		preparerSQL.addParametre(consignationRapportVO.getSite());
		
		query.append("AND c.i_dn_cle = tdn.l_tr_cle and tdn.i_la_cle = 1 ");
		...
		query.append("order by  c.v_cn_numero_serie, tdn.v_tr_description ");
